...
def gettag_coordinateNew(atoms, shift, tagD, xCenter, yCenter,xInterface2, yInterface2, boxlength):
    #Places tag based off of the distances provided Oct 2nd and the angle at which Interface2 is
    #rotated. Adds/subtracts difference for each tag from the angle of Interface2 
    tagX = np.zeros(atoms)
    tagY = np.zeros(atoms)
    for atom in range(atoms):
        
        Interface2Angle = np.arctan2(yInterface2[atom] - yCenter[atom], xInterface2[atom] - xCenter[atom])

        tagAngle = Interface2Angle + shift
...
def CalcRDF(atoms, x, y, box1D, dimL, edges):
    ##Calculate the RDF for the HS Center 
    dist = []
    for atom in range(atoms):
...
tagShift = [1.189256963875768, -2.0888099255464487, -2.580278545590712, 0.214600580275917, -0.370212396456965, -3.003407802113171,2.258465463709947, 1.2799470655235412 ]
        ...
        for tri in range(len(trials)):
            ...
            for snap in range(snaps):
                ...
                if snap in range(startSnap, snaps):
                    ...
                    Type2, xInterface2, yInterface2 = getTypeData(typ, x, y, boxL, 2)
                    Type1, xInterface1, yInterface1 = getTypeData(typ, x, y, boxL, 3)
                    
                    # Find tag locations for new position for each snap 
                    tagX65, tagY65 = gettag_coordinateNew(atoms[0], tagShift[0], tagDist[0],  xCenter, yCenter,xInterface2, yInterface2, boxL)
                    tagX90, tagY90 = gettag_coordinateNew(atoms[0], tagShift[1], tagDist[1],  xCenter, yCenter,xInterface2, yInterface2, boxL)
                    tagX93, tagY93 = gettag_coordinateNew(atoms[0], tagShift[2], tagDist[2],  xCenter, yCenter,xInterface2, yInterface2, boxL)
                    tagX188, tagY188 = gettag_coordinateNew(atoms[0], tagShift[3], tagDist[3], xCenter, yCenter,xInterface2, yInterface2, boxL)
                    tagX237, tagY237 = gettag_coordinateNew(atoms[0],tagShift[4], tagDist[4], xCenter, yCenter,xInterface2, yInterface2, boxL)
                    tagX451, tagY451 = gettag_coordinateNew(atoms[0],tagShift[5], tagDist[5], xCenter, yCenter,xInterface2, yInterface2, boxL)
                    tagX491, tagY491 = gettag_coordinateNew(atoms[0],tagShift[6], tagDist[6], xCenter, yCenter,xInterface2, yInterface2, boxL)
                    tagX534, tagY534 = gettag_coordinateNew(atoms[0], tagShift[7], tagDist[7], xCenter, yCenter,xInterface2, yInterface2, boxL)
